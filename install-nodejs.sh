#linux nodejs自动安装程序 
#运行例子：mkdir -p /shell && cd /shell && rm -rf install-nodejs.sh && wget --no-cache https://raw.githubusercontent.com/share-group/shell/master/install-nodejs.sh && sh install-nodejs.sh 8.9.0 /usr/local
ntpdate ntp.api.bz
...
if [ ! $nodejs_version ] || [ ! $nodejs_install_path ]; then
	...
	echo 'for example: sh install-nodejs.sh 8.9.0 /usr/local'
	exit
fi
