#linux nginx自动安装程序 
#运行例子：mkdir -p /shell && cd /shell && rm -rf install-nginx.sh && wget --no-cache https://raw.githubusercontent.com/share-group/shell/master/install-nginx.sh && sh install-nginx.sh 1.14.2 /usr/local
ntpdate ntp.api.bz
...
if [ ! $nginx_version ] || [ ! $nginx_install_path ]; then
	...
	echo 'for example: sh install-nginx.sh 1.14.2 /usr/local'
	exit
fi
