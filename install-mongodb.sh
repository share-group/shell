#linux mongodb自动安装程序 
#运行例子：mkdir -p /shell && cd /shell && rm -rf install-mongodb.sh && wget --no-cache https://raw.githubusercontent.com/ruanzhijun/share/master/shell/install-mongodb.sh && sh install-mongodb.sh 3.4.4 /usr/local
...
if [ ! $mongodb_version ] || [ ! $mongodb_install_path ] ; then
	...
	echo 'for example: sh install-mongodb.sh 3.4.4 /usr/local'
	exit
fi
