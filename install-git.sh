#linux git
#运行例子：mkdir -p /shell && cd /shell && rm -rf install-git.sh && wget --no-check-certificate --no-cache https://raw.githubusercontent.com/share-group/shell/master/install-git.sh && sh install-git.sh /usr/local
