#linux gitlab
#运行例子：mkdir -p /shell && cd /shell && rm -rf install-gitlab.sh && wget --no-cache https://raw.githubusercontent.com/ruanzhijun/share/master/shell/install-gitlab.sh && sh install-gitlab.sh
#定义本程序的当前目录
base_path=$(pwd)
ntpdate ntp.api.bz
